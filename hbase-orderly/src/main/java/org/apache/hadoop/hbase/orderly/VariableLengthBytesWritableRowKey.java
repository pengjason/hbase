...
/**
 * Serializes and deserializes BytesWritable into a sortable variable length
 * representation with an optional fixed length prefix (on which no encoding
 * will be applied).
 *
 * <h1>Serialization Format</h1>
 * <p/>
 * Variable length byte arrays can not be written simply as the original byte
 * array, because this would leave us without any usable byte (or byte
 * sequence) to mark the end of the byte array. We can also not simply write
 * the length of the array in a fixed number of bytes in the beginning of the
 * output, because this would mean the rowkeys can no longer be sorted and
 * "prefix searched". This impacts things like splitting tables over regions
 * etc.
 * <p/>
 * Therefore we use a variant of packed binary coded decimal (BCD). We start by
 * interpreting the byte array as an unsigned variable length integer number
 * represented in decimal format (using only digits 0 - 9). Packed BCD encodes
 * each digit (0 - 9) into a 4 bit nibble (binary 0000 - 1001) and "packs" two
 * of these nibbles into one byte.
 * <p/>
 * BCD only uses the byte range (binary) 00000000 - 10011001. This results in
 * an uneven distribution of values over the whole possible range, resulting
 * in problems with splitting tables over regions based on predefined byte
 * ranges for each region. To solve this, we modified BCD with a mapping from
 * digits to 4 bit values which uses the range more evenly. Furthermore, we
 * reserve a number of small nibbles for special purposes.
 * <p/>
 ...
 * </pre>
 *
 * This also means that a number of values are "available" as terminator,
 * filler and to encode NULL values , we've chosen the nibble 0000 for NULL
 * values and the nibble 0001 as terminator. This ensures that shorter byte
 * arrays that are the prefix of a longer byte array will always compare less
 * than the longer string, as the terminator is a smaller value that any
 * decimal digit.
 * <p/>
 * Furthermore, we also want that if we encode two byte arrays of the same
 * length, that the resulting encoded byte arrays are also of the same length
 * and that their byte order is the same as the byte order of the original byte
 * arrays. Also, when encoding two byte arrays of different lengths, the byte
 * order of the encoded byte arrays should be the same as of the original byte
 * arrays. To accomplish this, we use a filler character such that all encoding
 * is always using the same number of encoded bytes per original byte (i.e. 3
 * nibbles per original byte, so a loss of 50%). The filler nibble should be
 * smaller than any real encoded value, but bigger than the nibbles used for
 * zero and NULL.
 * <p/>
 ...
 * operations.
 * <p/>
 * The fixed length prefix will be written as is before the variable length
 * part. This can be useful to prevent BCD encoding on a fixed part (which you
 * know will always be there anyways).
 *
...
public class VariableLengthBytesWritableRowKey extends RowKey {
  ...
  public int getSerializedLength(Object o) throws IOException {
    ...
    return fixedPrefixLength
        + getSerializedLength(toStringRepresentation(input.getBytes(), fixedPrefixLength,
          input.getLength() - fixedPrefixLength));
  }

  /**
   * @return the length of a String with digits if serialized in our
   *         customized BCD format. We require 1 byte for every 2 characters,
   *         rounding up. Furthermore, if the number of characters is even,
   *         we require an additional byte for the terminator nibble if
   *         terminate() is true.
   */
  ...
  @Override
  public void serialize(Object o, ImmutableBytesWritable bytesWritable)
      throws IOException {
    ...
    if (o == null) {
      if (fixedPrefixLength > 0)
        throw new IllegalStateException("excepted at least "
            + fixedPrefixLength + " bytes to write");
      else if (terminate()) {
      ...
      if (fixedPrefixLength > input.getLength())
        ...
        encodedCustomizedReversedPackedBcd(
          toStringRepresentation(input.getBytes(), fixedPrefixLength, input.getLength()
              - fixedPrefixLength), bytesWritable);
      }
    }
  }
  ...
  private String toStringRepresentation(byte[] bytes, int offset, int length) {
    ...
    for (int i = 0; i < length; i++) {
      ...
      // An unsigned byte results in max 3 decimal digits (because max
      // value is "255") and we want to use always the max size such that
      // if two byte arrays have the same length, the two encoded byte
      // arrays also have the same length (such that byte sort order is
      // remained). Therefore, we fill the gaps with a filler character.
  ...
  /**
   * Prepend a string with zeroes up to the given total length. Note that we
   * could have used String.format("%03d", ...) here but this turned out to
  ...
  private byte[] fromStringRepresentation(String string) {
    ...
    for (int i = 0; i < result.length; i++) {
      ...
      for (int j = 0; j < 3; j++) {
        singleByteBcdString.append(
          digits[digitIdx + j] == FILLER ? "" : digits[digitIdx + j]);
      }
  ...
  protected int getBcdEncodedLength(byte[] bytes, int offset, int len) {
    ...
    while (i < len) {
      ...
      if ((c & 0x0f) == TERMINATOR_NIBBLE) break;
    }
  ...
  private static byte[] CUSTOMIZED_BCD_ENC_LOOKUP =
      new byte[]{3, 4, 5, 6, 7, 9, 10, 12, 14, 15};

  // note that the value -1 means invalid
  private static byte[] CUSTOMIZED_BCD_DEC_LOOKUP =
      new byte[]{-1, -1, -1, 0, 1, 2, 3, 4, -1, 5, 6, -1, 7, -1, 8, 9};

  /**
   * Encodes a String with digits into a "customized reversed packed binary
   * coded decimal" byte array.
   */
  void encodedCustomizedReversedPackedBcd(String decimalDigits,
      ImmutableBytesWritable bytesWritable) {
    byte[] bytes = bytesWritable.get();
    int offset = bytesWritable.getOffset();
    ...
    for (int i = 0; i < encodedLength; i++) {
      // initialize with termination nibbles
      byte bcd = TWO_TERMINATOR_NIBBLES;
      int digitsIdx = 2 * i;
      ...
      if (++digitsIdx < digits.length) {
        ...
        // uneven number of digits -> write terminator nibble
        bcd |= TERMINATOR_NIBBLE;
      }
      // this could be two bcd nibbles or two terminator nibbles
      bytes[offset + i] = mask(bcd);
    }
  ...
  private byte lookupDigit(char digit) {
    ...
    else return FILLER_NIBBLE;
  }

  /**
   * Decodes a "customized reversed packed binary coded decimal" byte array
   * into a String with digits.
   *
  ...
  protected boolean addDigit(byte bcd, StringBuilder sb) {
    if (bcd == TERMINATOR_NIBBLE) {
      ...
      if (bcd != FILLER_NIBBLE) sb.append(CUSTOMIZED_BCD_DEC_LOOKUP[bcd]);
