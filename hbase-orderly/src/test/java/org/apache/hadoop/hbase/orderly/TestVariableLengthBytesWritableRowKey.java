...
import org.apache.hadoop.hbase.orderly.VariableLengthBytesWritableRowKey;
...
public class TestVariableLengthBytesWritableRowKey extends RandomRowKeyTestCase {
  ...
  private byte[] encode(String decimalDigits, int expectedLength) {
    final ImmutableBytesWritable result =
        new ImmutableBytesWritable(new byte[expectedLength]);
    new VariableLengthBytesWritableRowKey().encodedCustomizedReversedPackedBcd(
  ...
  public void testEncode() {
    ...
    // a special case (encoded value is 0x9E which is actually a negative byte
    // when interpreted as signed value)
  ...
  public void testDecode() {
    ...
    // stuff after termination nibble (0x01) is ignored
    assertEquals("123", decode(new byte[] { 0x45, 0x61, 0x00 }));
  }
}
